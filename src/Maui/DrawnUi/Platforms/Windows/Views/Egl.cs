...
namespace DrawnUi.Views
{
    ...
    /// <summary>
    /// Custom EGL wrapper using advanced library loading to replace SkiaSharp internal Egl
    /// </summary>
    public static class Egl
    {
        ...
        /// <summary>
        /// Static constructor that ensures EGL library is properly loaded
        /// </summary>
        static Egl()
        {
            try
            {
                // Pre-load the library but let DllImport handle the actual binding
                bool loaded = DrawnUi.Infrastructure.NativeLibraryLoader.TryLoadNativeLibrary(libEGL+".dll");
                if (!loaded)
                {
                    System.Diagnostics.Debug.WriteLine("Warning: Could not pre-load EGL library: libEGL.dll");
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine("Loaded libEGL.dll OK");
                    LibLoaded = true;
                }
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Error pre-loading EGL library: {ex.Message}");
            }
        }

        public static bool LibLoaded;
        ...
        public const int EGL_PIXMAP_BIT = 0x0002;   // Supports pixmap surfaces  
        public const int EGL_WINDOW_BIT = 0x0004;   // Supports window surfaces
        ...
        [DllImport(libEGL)]
        public static extern glbool eglGetConfigAttrib(EGLDisplay dpy, EGLConfig config, int attribute, out int value);
