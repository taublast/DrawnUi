...
namespace DrawnUi.Views
{
    public class GlesContext : IDisposable
    {
        ...
        public void CreateSurface(SwapChainPanel panel, Size? renderSurfaceSize, float? resolutionScale)
        {
            ...
            if (!panel.IsLoaded)
            {
                throw new InvalidOperationException("SwapChainPanel must be loaded before creating surface");
            }

            if (Egl.eglGetConfigAttrib(eglDisplay, eglConfig, Egl.EGL_SURFACE_TYPE, out int surfaceType) == Egl.EGL_TRUE)
            {
                if ((surfaceType & Egl.EGL_WINDOW_BIT) == 0)
                {
                    throw new Exception("EGL config doesn't support window surfaces");
                }
            }
            ...
            {
                throw new Exception("Failed to convert PropertySet to IInspectable");
            }

            surface = Egl.eglCreateWindowSurface(eglDisplay, eglConfig, inspectable.ThisPtr, surfaceAttributes);
        ...
        {
            OpenGles.Viewport(0, 0, width, height);
        }
