...
public partial class SkiaColorPicker : SkiaLayout
{
    ...
    public virtual void SetupIndicator()
    {
        ...
        Indicator.TranslationX = (DrawingRect.Width * PointerRingPositionXOffsetRatio) / RenderingScale -
                                 PointerRingDiameterUnits / 2f;
        Indicator.TranslationY = (DrawingRect.Height * PointerRingPositionYOffsetRatio) / RenderingScale -
                                 PointerRingDiameterUnits / 2f;

        Trace.WriteLine($"Indicator at {PointerRingPositionXOffsetRatio}, {PointerRingPositionYOffsetRatio}");
    }
    ...
    public override ISkiaGestureListener ProcessGestures(SkiaGesturesParameters args, GestureEventProcessingInfo apply)
    {
        if (args.Type == TouchActionResult.Down || args.Type == TouchActionResult.Panning)
        {
            ...
            Trace.WriteLine($"P {PointerRingPositionXOffsetRatio}");
    ...
    public static readonly BindableProperty ColorsBlendModeProperty = BindableProperty.Create(nameof(ColorsBlendMode),
        ...
        SKBlendMode.SrcOver,
        propertyChanged: (bindable, value, newValue) => { ((SkiaColorPicker)bindable).UpdateColors(); });
    ...
    public static readonly BindableProperty PointerRingDiameterUnitsProperty
        = BindableProperty.Create(
            ...
            16.0,
            BindingMode.Default,
            validateValue: (bindable, value) => { return (((double)value > -1) && ((double)value <= 1)); },
            propertyChanged: (bindable, value, newValue) =>
            {
                if (newValue != null)
    ...
    protected virtual List<Color> GetSecondaryLayerColors(ColorSpectrumStyle colorSpectrumStyle)
    {
        switch (colorSpectrumStyle)
        {
            ...
            case ColorSpectrumStyle.Shades:
                return new() { Colors.White, Colors.Black };
            case ColorSpectrumStyle.HueToShadeStyle:
                return new() { Colors.Transparent, Colors.Black };
            case ColorSpectrumStyle.ShadeToHueStyle:
                return new() { Colors.Black, Colors.Transparent };
            case ColorSpectrumStyle.HueToTintStyle:
                return new() { Colors.Transparent, Colors.White };
            case ColorSpectrumStyle.TintToHueStyle:
                return new() { Colors.White, Colors.Transparent };
            case ColorSpectrumStyle.TintToHueToShadeStyle:
                return new() { Colors.White, Colors.Transparent, Colors.Black };
            case ColorSpectrumStyle.ShadeToHueToTintStyle:
                return new() { Colors.Black, Colors.Transparent, Colors.White };
            default:
                return new() { Colors.Transparent, Colors.Black };
        }
    }
