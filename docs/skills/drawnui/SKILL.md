...
- **Context menu on the web heads (2026-09-06, AppoMobi.Gestures 3.11.1)**: a right click / long press / Menu key over the canvas calls `SkiaControl.ContextMenu`, a `Func<SkiaControl, ContextMenuEventArgs, bool>` delegate (not an event, like `OnGestures`; args: `Source`, `Local`), fluent `.OnContextMenu((v, e) => { …; return true; })`. Routed like a tap through `ProcessGestures` as `TouchActionResult.ContextMenu`; `return true` suppresses the browser's own canvas menu, false / null = browser menu shows (Blazor used to swallow it always). Right / middle clicks never `Tapped` on Blazor / Wasm. Other platforms never raise it. Same API in DrawnUi.React (`ContextMenu={(s, e) => true}`).
...
- **Windows accelerated canvas under animated XAML scale transforms** (root-caused + fixed in lib 2026-08-28, ≥ 1.10.5.19): `RenderingMode=Accelerated` on Windows is a WinUI `SwapChainPanel`. `CompositionScale` = DPI × every ancestor scale transform; SkiaSharp's `AngleSwapChainPanel` recreates the EGL surface on EVERY `CompositionScaleChanged` sized by that value, so during a popup `ZoomIn` (FastPopups) the GL buffer is half-size while DrawnUI paints at DPI → the animated frames show a 2× zoomed crop of the page (last frame correct), and the mid-animation recreation detaches the swap-chain visual from the running animation (canvas-side compensation therefore cannot fix it — tried, rejected). Fix: DrawnUi now owns the panel — `Platforms/Windows/Views/DrawnSwapChainPanel.cs` (MIT fork of `AngleSwapChainPanel`) + `Views/Gles/{Egl,Gles,GlesContext}.cs`: surface always created with `resolutionScale = XamlRoot.RasterizationScale`, `CompositionScaleChanged` ignored while `CompositionScale ≠ RasterizationScale` (transform in flight), recreated only on a real DPI change. `SKSwapChainPanelRetained`/`SKSwapChainPanel` derive from it. Two traps: (1) ANGLE reads `EGLRenderResolutionScaleProperty` via WinRT `IPropertyValue` — a CsWinRT-boxed `float`/`Size` crashes `eglCreateWindowSurface` (0xC0000005); SkiaSharp's C++/WinRT helper `SkiaSharp.Views.WinUI.Native.PropertySetExtensions.AddSingle/AddSize` (runtime-only projection in `SkiaSharp.NativeAssets.WinUI`) is bound by reflection, with a logged fallback to stock behaviour; (2) a dead 2025-07 fork (`Views/GlesContext.cs`, `Egl.cs`, `AngleAcceleratedView.cs`, `PropertySetExtensions.cs` → never-shipped `SkiaSharp.DrawnUi.WinUI.Native.dll`) used to sit in namespace `DrawnUi.Views` and hijacked type resolution (same-namespace types beat `using` aliases) — deleted 2026-08-28; the live glue is `DrawnUi.Views.Gles.*`, referenced `global::`-qualified from the panel. Diagnostic: log `((SwapChainPanel)canvas.CanvasView.Handler.PlatformView).CompositionScaleX` vs `canvas.RenderingScale` per frame in `WasDrawn`; `PrintWindow` captures do NOT apply animated transforms — judge with eyes. Verified visually on ArtOfFoto light-table popup (accelerated).
...
### SkiaShell hosting + modal contract (verified 2026-09-06, FiltersCamera migration, Windows accelerated)

- `SkiaShell` IS the page (`SkiaShell : BasePageReloadable : ContentPage`), not a control: subclass it, build in `Build()`, and set `Content` to the `Canvas` DIRECTLY (`Initialize` reads `Content as Canvas`; a `Grid` wrapper breaks it). Tree: `Canvas > SkiaLayer{Tag="ShellLayout"}.Fill() > [SkiaControl{Tag="RootLayout"} placeholder, ...]` — the `.Fill()` on the shell layer is REQUIRED: without it the layer measured ~55pt tall and every modal/screen was clipped to a top band (a `SkiaLayer` alias does not fill vertically by itself here).
- Startup route: `Initialize("")` accepts an inline RootLayout, BUT a shell with no root node makes `GoToAsync("anything")` call `SetRoot("anything")` (throws for a non-page route). Register a root TYPE (`RegisterRoute("root", typeof(RootScreen))`, parameterless ctor, tag its `SkiaViewSwitcher` `"NavigationLayout"`) and `Initialize("root")`; then `RegisterActionRoute("myshots", ...)` + `GoToAsync("myshots")` and modal routes (`Shell.SetPresentationMode(this, PresentationMode.ModalAnimated)` in the control ctor, `GoToAsync("help")`) all work, and hot reload re-creates the root through the same route.
- Only `PushModalAsync(content, useGestures, animated, freezeBackground:true)` stops rendering below: `ModalWrapper` snapshots its `SkiaBackdrop`, the shell puts a `SkiaImage` of it on `ShellLayout` and sets `RootLayout.IsVisible=false` (animators under it get `IsHiddenInViewTree`). `OpenPopupAsync(..., showOverlay:true)` does NOT freeze — `PopupWrapper` only hides its backdrop and tints itself; everything under a popup keeps rendering.
- Modal = `SkiaDrawer` FromBottom over a Fill wrapper: the whole content slides up; `useGestures:true` = swipe-down closes. Give sheet-style content a Fill layer with a `SkiaHotspot` (tap outside → close) and the sheet `VerticalOptions=End`; keep `useGestures:false` when the content pans itself (carousel, divider drag, scroll).
- Result callbacks: implement `IVisibilityAware` on the modal CONTENT — `OnDisappearing` fires once from `RemoveModal` after the drawer closed for EVERY close path (button, tap outside, swipe, `GoBack`), so deliver the result there (guard once, marshal to main thread). Close your own modal via `PopModalAsync(PageInStack, animated)` found by `NavigationStackModals.FirstOrDefault(x => x.Page is ModalWrapper w && w.Drawer?.Content == this)`; with `animated:true` it removes the entry from the stack immediately and the wrapper is torn down later, so "is X open" = scan `NavigationStackModals` wrappers' `Drawer.Content`. `PopModalAsync(bool)` on a modal that never became visible returns null and leaves it in the stack → bound any "pop everything" loop.
- Back: set `SkiaShell.OnShellGoBack = page` (`IHandleGoBack`); `GoBack(animate)` asks it first (true = consumed), then `GoBackDefault` = popups → modals → pushed screens. Android: override `MainActivity.OnBackPressed` → `shell.GoBack(true)` else `base`.
- Frozen snapshot rendered BLACK on an accelerated canvas: the backdrop snapshot is texture-backed and `SkiaImage.Blur` (image filter) through the raster `Image` cache of the frozen layer draws nothing (same image without the filter draws fine). Fixed in lib `e2c768dc` (`WrapScreenshot` rasterizes via `LoadedImageSource.DrawImageOnCpuSurface` once); on NuGet ≤ 1.10.6.9 override `WrapScreenshot` in the shell subclass and do the same before calling base. Lib nit still there: `SkiaShell.Dispose` does `Super.InsetsChanged += OnInsetsChanged` (should be `-=`).
- Hot Preview under a shell: `[Preview]` void methods should `await CloseAllPopups()` + bounded `PopModalAsync(false)` loop, then run the action on the main thread; route-created overlays with public parameterless ctors get auto-previewed as bare controls — mark them `[AutoGeneratePreview(false)]`.
- Modal slide speed (fixed 2026-09-06, lib `b5ab5a23`): `SkiaDrawer.GetAutoVelocity` was a hardcoded 1500 pt/s, which `SnappingLayout` turns into `0.7 * clamp(300/v, 0.1, 0.8)` = 0.14 s — a programmatic `IsOpen` change read as a pop-in. Now `SkiaDrawer.AutoVelocity` (default 1500, so nothing else changes) and `ModalWrapper` sets it from `SkiaShell.PopupsAnimationSpeed` (840 pt/s for 250 ms). Diagnosing "no animation" claims: log inside `SnappingLayout.ScrollToOffset` (speed + velocity) rather than reasoning from `Animated && WasDrawn` — that gate was true; the duration was the problem. Screen bursts on Windows: `desktop_region_screenshot` through the hot-preview DevTools MCP works when `CopyFromScreen` from a scripted PowerShell fails with "The handle is invalid".
- Pushed pages via `PushAsync` are DISPOSED by `SkiaViewSwitcher.PopPage` (`preserve:false` from the shell) — never keep and re-push an instance, create a new one per push. `NavigationStackScreens` holds them; `GoBackDefault` pops them after popups and modals.
- `SkiaLabel` built-in `DropShadowSize/OffsetX/OffsetY` did not count toward the effects margin (a cached label clipped the shadow on the left/top when size > offset); fixed lib `a0e5e7ea` (`ComputeEffectsMargin` override). On older packages use `ExpandDirtyRegion` on the label.
...
- Store a small thumb (≈400px) at save time and bind the tile to it; keep the display copy for the viewer.
- Set `LoadSourceOnFirstDraw = true` on tile images so measure-pass binds do nothing and only drawn tiles decode.
- A cache hit still costs `SKImage.FromBitmap(cached)` (a copy) per bind, ~8 ms for 1080p — another reason for thumbs.

## SkiaViewSwitcher transitions (verified 2026-09-06)

- `AnimateTabs` defaults to `false` — tab switches (`SelectedIndex`) pop in with no animation unless you set it; `AnimatePages` (push/pop) defaults to true.
- Transitions are time-based (`TranslateToAsync`, `PagesAnimationSpeed` 200 ms). Lib ≥ `bb8965b0` waits for the switcher's next rendered frame before animating (`WaitForNextFrameAsync`), otherwise a heavy page's first measure/paint ate the whole animation and the page popped in. Keep the pushed page's first frame cheap anyway (see SkiaImage rule above).
- Custom directions: subclass and override `SetupTransitionAnimation` / `ExecuteTransitionAnimation` (both `protected virtual`, receive `DoubleViewTransitionType` Push/Pop/SelectLeftTab/SelectRightTab from `DrawnUi.Infrastructure.Enums`). Stock setup only touches `TranslationX`; reset `TranslationY` yourself when mixing vertical and horizontal. `RevealNavigationView` zeroes both translations and opacity after the animation. FiltersCamera `AppNavigation`: transitions touching child 0 (camera) slide vertically, everything else stock horizontal.
- Diagnosing "no animation" without guessing: burst `desktop_region_screenshot` frames around the trigger and look for intermediate positions; add a `Rendered` handler on the new view to log first-frame latency vs animation length.
...
- **Templated layout resizing OUTSIDE a scroll** (fixed 2026-09-05, lib commit `0ed6a5a3`): structure-preserving collection changes (MeasureFirst uniform add/remove `TryApplyUniformAddMeasureFirst`, bg-measure batches, visibility) resize the layout via `SetMeasured` in `UpdateProgressiveContentSize` with NO measure pass. `SkiaScroll.Draw` POLLS `Content.MeasuredSize` every frame (`_lastContentSize != Content.MeasuredSize` → adopts + `ApplyContentSize`), so scroll-hosted lists never needed a push. A plain autosized parent (SkiaShape/SkiaLayer/grid cell) arranges the child at the size from ITS OWN last measure pass → first item shows, later ones stay clipped (ArtOfFoto exposure results). Same class: head insert/remove two-phase rebase (`_pendingHeadRemove`) was committed only by `SkiaScroll.Draw` → under a plain parent never committed, `HeadRemoveInFlight` stuck true. Fix: `NotifyParentOfArithmeticResize()` = `InvalidateParent()` when `NeedAutoSize && Parent is not IDefinesViewport` (cheap: `SkiaControl.Measure` short-circuits on `!NeedMeasure` + same constraints, parent just adopts the size); `SkiaLayout.Paint` commits `CommitPendingStructureRebase()` itself when parent is not a viewport. Tells: `stack.MeasuredSize.Pixels.Height` grows while `stack.DrawingRect.Height` stays; `HasPendingStructureChanges` true forever after a `RemoveAt(0)`. Diagnostic trap: `VerticalOptions=Fill` on the stack masks it. Headless repro: `SkiaLayer > SkiaShape(Start/Start, GPU) > SkiaStack(templated, Start/Start)` + `ObservableCollection.Add/RemoveAt(0)` — baseline showed measured 16→40→64 with shape stuck at 24.
